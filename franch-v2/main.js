...
(() => {
    ...
    function initModal() {
        ...
        const defaultTitle    = modalTitleEl ? modalTitleEl.textContent.trim() : '';
        const defaultSubtitle = modalSubtitleEl ? modalSubtitleEl.textContent.trim() : '';

        // Each CTA can carry its own promise (data-cta-title/data-cta-subtitle) so the
        // popup reflects what the visitor actually clicked, instead of one generic
        // "получить материалы" form regardless of entry point.
        const openModal = (formatName, ctaTitle, ctaSubtitle) => {
            ...
            if (modalSubtitleEl) modalSubtitleEl.textContent = ctaSubtitle || defaultSubtitle;
            if (modalFormatBadge) {
                if (formatName) {
                    modalFormatBadge.textContent = 'Формат: ' + formatName;
                    modalFormatBadge.hidden = false;
                } else {
                    modalFormatBadge.hidden = true;
                }
            }
            setTimeout(() => {
        ...
        document.querySelectorAll('a[href="#capture"]').forEach((a) => {
            a.addEventListener('click', (e) => {
                e.preventDefault();
                openModal('', a.dataset.ctaTitle, a.dataset.ctaSubtitle);
            });
        });

        // Intercept format-card CTAs (pass format name + contextual title)
        document.querySelectorAll('[data-cta-format]').forEach((btn) => {
            btn.addEventListener('click', () => openModal(btn.dataset.formatName || '', btn.dataset.ctaTitle, btn.dataset.ctaSubtitle));
        });
    ...
    async function submitLead(form, successEl) {
        ...
        try {
            ...
            // Silent fail if server isn't ready — local jsonl on the server is the
